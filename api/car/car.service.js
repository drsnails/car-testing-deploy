...
async function remove(carId, loggedinUser) {
    try {
        const carToRemove = await getById(carId)
        if (!loggedinUser.isAdmin && carToRemove?.owner?._id !== loggedinUser._id) throw 'Cant remove car'
...
async function save(carToSave, loggedinUser) {
    try {
        if (carToSave._id) {
            if (!loggedinUser.isAdmin && carToSave?.owner?._id !== loggedinUser._id) throw 'Cant save car'
